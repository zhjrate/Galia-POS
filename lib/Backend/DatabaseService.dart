...
class DatabaseService {
  ...
  final CollectionReference masterData = FirebaseFirestore.instance.collection('ERP').doc('VTam7iYZhiWiAFs3IVRBaLB5s3m2').collection('Master Data');
  ...
  Stream <List<SavedOrders>> orderList () async*{
    yield*  FirebaseFirestore.instance.collection('ERP').doc('VTam7iYZhiWiAFs3IVRBaLB5s3m2').collection('Saved').snapshots().map(_savedOrderListFromSnapshot);
  }
  ...
  //Create Order
  Future createOrder(String year, String month, String transactionID, subTotal, discount, tax, total, orderDetail, orderName, paymentType) async {

    // final User user = FirebaseAuth.instance.currentUser;
    // final String uid = user.uid.toString();

    return await FirebaseFirestore.instance.collection('ERP')
  ...
  //Create Order
  Future saveOrder(String transactionID, subTotal, discount, tax, total, orderDetail, orderName, paymentType, orderColor) async {
  ...
  Future deleteOrder (orderDoc) async {
    ...
    return await FirebaseFirestore.instance.collection('ERP')
      .doc('VTam7iYZhiWiAFs3IVRBaLB5s3m2').collection('Saved').doc(orderDoc).delete();
  } 
  ...
  Stream <CategoryList> get categoriesList async*{
    yield* masterData.doc('Categories').snapshots().map(_categoriesFromSnapshot);
  }
  ...
  AccountsList _accountsFromSnapshot (DocumentSnapshot snapshot){
    ...
    try{
      return AccountsList(        
        gastosdeEmpleados: snapshot.data()['Gastos de Empleados'].map<Categories>((item){
          return Categories(
            category: item['Category'] ?? '',
            productDescription: item['Descripción'] ?? '',        
            vendors: item['Vendors'] ?? [],
          );
        }).toList(),
        gastosdelLocal: snapshot.data()['Gastos del Local'].map<Categories>((item){
          return Categories(
            category: item['Category'] ?? '',
            productDescription: item['Descripción'] ?? '',     
            vendors: item['Vendors'] ?? [],
  ...
  Stream <AccountsList> get accountsList async*{
    yield* masterData.doc('Account Mapping').snapshots().map(_accountsFromSnapshot);
  }

    //Create Order
  Future saveExpense(costType, category, account, vendor, product, qty, price, total, paymentType) async {
  ...
  Future saveExpenseType(costType, category, account, amountCostType, amountAccount, amountCategory) async {
  ...
  Stream <List<Expenses>> expenseList () async*{
    ...
    yield*  FirebaseFirestore.instance.collection('ERP')
    .doc('VTam7iYZhiWiAFs3IVRBaLB5s3m2')
    .collection(year)
    .doc(month)
    .collection('Expenses')
    .snapshots().map(_expensesFromSnapshot);
  }
